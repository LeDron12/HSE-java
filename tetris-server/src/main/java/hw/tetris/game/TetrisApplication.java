...
import hw.tetris.server.GameServer;
...
public class TetrisApplication extends Application {
    ...
    private Parent createWaitingView() {
        Pane mainPane = new Pane();
        mainPane.setPrefSize(275, 100);

        Label waitingLabel = new Label();
        waitingLabel.setText("Ожидание всех игроков...");
        waitingLabel.setLayoutX(70);
        waitingLabel.setLayoutY(47);

        mainPane.getChildren().add(waitingLabel);
        return mainPane;
    }
...
//                mainPane.getChildren().add(new Figure(placed).getFigure());
                mainPane.getChildren().add(new Figure(placed, GameServer.figureSeeds[placed]).getFigure());
                placed++;
    ...
    private EventHandler<? super MouseEvent> onGenFigureButtonClick(Pane pane) {
        pane.getChildren().add(new Figure(placed, GameServer.figureSeeds[placed]).getFigure());
        placed++;
        return null;
    }
    ...
    private void configureBottomPane(Pane bottomPane) {
        Label opponentNameLabel = new Label();
        opponentNameLabel.setText("Имя опонента: " + opponentName);
        opponentNameLabel.setLayoutX(200);
        opponentNameLabel.setLayoutY(25);
        bottomPane.getChildren().add(opponentNameLabel);
    }
    ...
    private void setSides(BorderPane borderPane) {
        ...
        configureBottomPane(bottomPane);
    ...
    public void start(Stage primaryStage) throws IOException {
        ...
        primaryStage.setScene(new Scene(createWaitingView()));
        primaryStage.show();
    }
